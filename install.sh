...
REPO="${IATCODER_REPO:-https://github.com/martin-los/iatcoder.git}"
INSTALL_DIR="${IATCODER_INSTALL_DIR:-$HOME/.iatcoder-agent}"
BRANCH="${IATCODER_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[iatcoder]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[iatcoder]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[iatcoder]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[iatcoder] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
    printf "\n${BOLD}╔══════════════════════════════════════════╗${RESET}\n"
    printf   "${BOLD}║        iatcoder  一键安装                    ║${RESET}\n"
    printf   "${BOLD}╚══════════════════════════════════════════╝${RESET}\n\n"
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    if [[ -d "$INSTALL_DIR/.git" ]]; then
        ...
        info "克隆 iatcoder 到 ${INSTALL_DIR} ..."
        rm -rf "$INSTALL_DIR"
    ...
    BIN_DIR="${IATCODER_BIN_DIR:-$HOME/.local/bin}"
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/iatcoder"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/iatcoder" "\$@"
EOF
    chmod +x "$LAUNCHER"

    printf "\n"
    success "iatcoder 安装完成！"
    printf "\n"
