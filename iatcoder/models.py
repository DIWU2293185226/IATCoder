...
class DeepSeekModelClient:
    """适配 DeepSeek API（OpenAI 兼容的 /chat/completions 接口）。

    DeepSeek 使用标准 OpenAI Chat Completions 格式，与 Anthropic 和
    OpenAI Responses API 格式不同，因此单独实现。
    """
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        for attempt in range(attempts):
            try:
                ...
                if exc.code >= 500 and attempt < attempts - 1:
                    ...
                    continue
                raise RuntimeError(f"DeepSeek API request failed with HTTP {exc.code}: {body}") from exc
        ...
        try:
            ...
            raise RuntimeError(
                "DeepSeek API error: backend returned non-JSON content that could not be parsed"
            ) from exc
        if data.get("error"):
        ...
        if not text:
            raise RuntimeError("DeepSeek API error: could not extract text from response")
        return text
...
class OpenAICompatibleModelClient:
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        在 agent 链路里的位置：
        它位于 `IATCoder.ask()` 的模型调用阶段，是稳定前缀缓存复用链路真正
        落到 provider API 的地方。
        """
        self.last_completion_metadata = {}
        payload = {
