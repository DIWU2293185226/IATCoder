...
# 持久的锁文件 fd 映射，key 是 memory_dir 的字符串路径。
# fd 保持打开状态就是为了让 OS 级文件锁在整个 dream 期间有效。
_lock_fds: dict = {}


def _os_try_lock(fd):
    """尝试对已打开的 fd 加 OS 级互斥锁，非阻塞。

    Unix 用 fcntl.flock，Windows 用 msvcrt.locking。
    锁在 fd 关闭时自动释放，不会因为进程崩溃而残留。
    """
    try:
...
def try_acquire_lock(memory_dir):
    ...
    # 先拿 OS 级锁，再检查持有者是否存活。
    # 顺序不能反：不加 OS 锁就检查，仍然是 check-then-act 竞态。
    ...
    try:
        ...
        if age < HOLDER_STALE_S and holder_pid > 0:
            try:
    ...
    # 写入自己的 PID，文件内容保持人类可读。
    os.lseek(fd, 0, os.SEEK_SET)
    os.ftruncate(fd, 0)
    os.write(fd, str(current_pid).encode("utf-8"))
    os.fsync(fd)

    # fd 保持打开，锁在 release_lock() 中关闭 fd 时自动释放。
    _lock_fds[str(memory_dir)] = fd
    return True


def release_lock(memory_dir):
    """释放锁并更新 mtime 作为 consolidation 时间戳。"""
    lock_path = _lock_path(memory_dir)
    key = str(memory_dir)
    ...
    try:
        timestamp = time.time()
        if fd is not None:
            # 先更新内容，再更新 mtime，最后关闭 fd 释放 OS 锁。
            os.lseek(fd, 0, os.SEEK_SET)
            os.ftruncate(fd, 0)
            os.write(fd, b"released")
            os.fsync(fd)
            os.utime(fd, (timestamp, timestamp))
            _os_unlock(fd)
            os.close(fd)
        else:
            # fd 已丢失（异常路径走到这里），回退到 path 操作。
...
You are performing a dream: a reflective pass over Iatcoder's memory files. Synthesize recent signal into durable, well-organized memory files so future sessions can orient quickly.
...
def run_dream(agent, quiet=False, session_ids=None):
    ...
    try:
        ...
        memory_scope = Path(".iatcoder") / "memory"
    dream_agent = Iatcoder(
...
def maintain_memory_after_turn(agent, final_answer):
    ...
    def _background_dream():
        try:
            ...
            # 先 release_lock 关闭 fd 释放 OS 锁（会更新 mtime 为当前时间），
            # 再手动将 mtime 恢复到 dream 开始之前，避免一次失败的 dream 重置 consolidation 计时器。
            release_lock(agent.memory_dir)
    ...
    thread = threading.Thread(target=_background_dream, name="iatcoder-auto-dream", daemon=True)
...
def normalize_memory_state(state, workspace_root=None):
    ...
    durable_root = Path(workspace_root) / ".iatcoder" / "memory" if workspace_root is not None else None
...
def retrieval_candidates(state, query, limit=3, workspace_root=None):
    ...
    if workspace_root is not None:
        durable_store = DurableMemoryStore(Path(workspace_root) / ".iatcoder" / "memory")
