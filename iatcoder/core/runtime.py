"""Agent runtime composition. Iatcoder 是运行时对象图的所有者.

Iatcoder 持有 session state / workspace / memory / checkpoints / persistence.
Engine 负责 turn-level 控制循环, 工具执行和模型输出解析在各自的 helper 模块.
"""
...
class Iatcoder(RuntimeSecretsMixin, RuntimeCheckpointsMixin):
    """Iatcoder 运行时: 整个 agent 的"对象图".

    __init__ 负责把所有子系统串联起来:
    - ModelClient: 和 LLM provider 通信
    - WorkspaceContext: 当前仓库快照
    - SessionStore: 会话持久化
    - LayeredMemory: 工作记忆 + 持久记忆
    - Engine: turn-level 控制循环
    - ContextManager: prompt 组装
    - PlanModeController / PermissionChecker / WorkerManager / TodoLedger 等

    ask() 是唯一的对外接口, 内部委托给 Engine.run_turn().
    """
    def __init__(
        self,
        model_client,
        ...
        self.run_store = run_store or RunStore(
            Path(workspace.repo_root) / ".iatcoder" / "runs"
        )
    ...
    def _resolve_memory_dir(self, memory_dir):
        if memory_dir:
            ...
            path = self.root / ".iatcoder" / "memory"
    ...
    def build_tools(self):
        """构建工具注册表. 所有可用工具 (read_file, run_shell, agent, ...) 在这里注册. """
    ...
    def build_prefix(self):
        ...
        text = textwrap.dedent(
            f"""\
            You are iatcoder, a small local coding agent working inside a local repository.
    ...
    def refresh_prefix(self, force=False):
        """检查 workspace 是否有变化, 必要时重建 prefix.

        如果 workspace fingerprint 没变, prefix 也不会变,
        这样 prompt cache 可以跨轮复用.
        """
    ...
    def _build_prompt_and_metadata(self, user_message):
        ...
        流程: refresh_prefix -> evaluate_resume_state -> context_manager.build
        如果超预算则自动 compact 后重试.
        元数据包含了 prefix/history/memory/workspace 的尺寸及缓存信息,
        供 trace/report 解释"这轮 prompt 是怎么拼出来的".
        """
    ...
    def update_memory_after_tool(self, name, args, result):
        ...
        # file_summaries 有两个作用: (1) 作为 prompt 上下文注入工作记忆,
        # (2) 作为 tool policy prior-read 的凭证.
        # 即使 memory 被关闭, 也必须维护 freshness, 否则 prior_read_required 策略会误拒写文件.
        if name == "read_file":
    ...
    def ask(self, user_message):
        """对外唯一入口. 委托给 Engine 完成一轮或多轮 model<->tool 循环. """
    ...
    def run_tool(self, name, args):
        """工具执行门面: 委托 tool_executor.run_tool() 完成校验/权限/执行. """
