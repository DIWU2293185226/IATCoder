...
class ContextManager:
    ...
    def build(self, user_message):
        ...
        在 agent 链路里的位置：
        它位于 `Iatcoder.ask()` 的每轮模型调用之前，是“真正发请求给模型”
        ...
        # 预算压缩循环: 如果 sum(sections) > total_budget, 按优先级反复裁剪.
        # 压缩顺序 (先牺牲的在前): relevant_memory > skills > history > memory > prefix.
        # 最新用户请求永远不被裁剪, 因为那是本轮最重要的输入.
        # 每个 section 有 floor, 降到底就不再降了.
