"""Turn-level runtime engine. 一轮用户请求的模型-工具控制循环.

Iatcoder 持有状态和持久化, Engine 持有控制逻辑:
一个用户请求 -> 多轮 model call -> tool execution -> 直到 final / step limit / error.
"""
...
class Engine:
    ...
    职责:
    - run_turn() 实现 model<->tool 交替循环
    - ask() 是 run_turn() 的同步封装 (消费事件, 返回 final answer)
    """
    ...
    def ask(self, user_message):
        """同步入口: 消费 run_turn() 生成的事件, 返回 final answer. """
        final_answer = ""
    ...
    def run_turn(self, user_message):
        """核心控制循环: 一个用户请求 -> 模型调用 -> 工具执行 -> 循环 -> final.

        循环流程图 (单次 tool step):

          context_manager.build(user_message)   # 1. 组装 prompt
                |
                v
          complete_model(client, prompt)        # 2. 调用 LLM
                |
                v
          model_output.parse(raw)              # 3. 解析模型输出
                |
                +--> kind=="tool" -> execute_tool_payload() -> 记录结果 -> 继续循环
                +--> kind=="final" -> 持久化, checkpoint, report -> 结束
                +--> kind=="retry" -> 记录提示, 继续循环

        终止条件: final 回答 / 达到 max_steps / provider 错误 / abort 请求.
        yield 事件供 ask() 或 TUI 消费.
        """
        agent = self.runtime
        ...
        # 记录用户消息到 session history 和 trace
        ...
        while tool_steps < agent.max_steps and attempts < max_attempts:
            ...
            # 第 3 步: 解析模型输出, 区分 tool / final / retry
            ...
            # 分支 B: 模型输出格式无效 -> 注入 retry 提示, 下一轮重新请求
            if kind == "retry":
                agent.record(
        ...
        # 循环结束: 超 step 限制或 retry 次数已达上限, 没有拿到 final.
        # fallback: 请求模型写一个优雅的进展摘要, 或直接输出停止消息.
