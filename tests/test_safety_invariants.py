...
from iatcoder import FakeModelClient, MiniAgent, SessionStore, WorkspaceContext
from iatcoder import cli as mini_cli
from iatcoder.task_state import TaskState
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        ...
        clear=True,
    ), patch("iatcoder.cli.OllamaModelClient", DummyModelClient):
        args = mini_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = mini_cli.build_agent(args)
...
def test_run_shell_uses_allowlisted_environment_only(tmp_path):
    ...
    if sys.platform == "win32":
        ...
        # Use single quotes inside the Python script to avoid conflict with outer double quotes.
        win_script = "import os; print(os.getenv('MCA_ALLOWLIST_SECRET', 'missing'))"
        command = f'"{sys.executable}" -c "{win_script}"'
    else:
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("iatcoder.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "iatcoder.runtime"

    with patch("iatcoder.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
