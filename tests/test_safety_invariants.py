...
from iatcoder import cli as iatcoder_cli
from iatcoder.core.task_state import TaskState
...
def test_cli_build_agent_wires_secret_env_names_from_parser(tmp_path):
    ...
    with patch.dict(os.environ, {"GITHUB_PAT": "ghp-1", "GH_PAT": "ghp-2"}, clear=True), patch(
        ...
        args = iatcoder_cli.build_arg_parser().parse_args(
            [
                "--cwd",
                str(tmp_path),
                "--approval",
        ...
        agent = iatcoder_cli.build_agent(args)
...
def test_cli_build_agent_uses_default_configured_secret_names(tmp_path):
    ...
    with patch.dict(os.environ, {"GH_PAT": "ghp-default-1"}, clear=True), patch(
        ...
        args = iatcoder_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = iatcoder_cli.build_agent(args)
...
def test_cli_build_agent_loads_project_env_secrets_before_redaction_setup(tmp_path):
    ...
    (tmp_path / "README.md").write_text("demo\n", encoding="utf-8")
    (tmp_path / ".env").write_text("IATCODER_DEEPSEEK_API_KEY=sk-project-secret\n", encoding="utf-8")
    with patch.dict(os.environ, {}, clear=True), patch("iatcoder.cli.AnthropicCompatibleModelClient", DummyModelClient):
        args = iatcoder_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
        agent = iatcoder_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["IATCODER_DEEPSEEK_API_KEY"]
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            "IATCODER_CUSTOM_SECRET": "custom-secret-value",
            "IATCODER_SECRET_ENV_NAMES": "IATCODER_CUSTOM_SECRET",
        },
        clear=True,
    ), patch("iatcoder.cli.OpenAICompatibleModelClient", DummyModelClient):
        args = iatcoder_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = iatcoder_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["IATCODER_CUSTOM_SECRET"]
...
def test_bound_tool_methods_call_tools_module(tmp_path):
    ...
    with patch("iatcoder.tools.registry.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "iatcoder.core.runtime"
