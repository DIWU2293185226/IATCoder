...
def test_patch_requires_prior_fresh_read_and_allows_after_read(tmp_path):
    ...
    rejected = agent.run_tool("patch_file", {"path": "README.md", "old_text": "world", "new_text": "iatcoder"})
    ...
    patched = agent.run_tool("patch_file", {"path": "README.md", "old_text": "world", "new_text": "iatcoder"})

    assert patched == "patched README.md"
    assert (tmp_path / "README.md").read_text(encoding="utf-8") == "hello iatcoder\n"
...
def test_rejected_patch_can_be_retried_after_informing_read(tmp_path):
    agent = build_agent(
        tmp_path,
        [
            '<tool name="patch_file" path="README.md"><old_text>world</old_text><new_text>iatcoder</new_text></tool>',
