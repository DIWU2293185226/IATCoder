...
from iatcoder.cli import handle_repl_command
from iatcoder.features import skills as skillslib
...
def build_agent(tmp_path, outputs):
    ...
    return Iatcoder(
        ...
        workspace=workspace,
        session_store=SessionStore(tmp_path / ".iatcoder" / "sessions"),
...
def test_prompt_documents_project_skill_frontmatter_contract(tmp_path):
    ...
    prompt = agent.prompt("Create .iatcoder/skills/audit/SKILL.md")

    assert "When creating Iatcoder skill files" in prompt
    assert ".iatcoder/skills/<name>/SKILL.md" in prompt
...
def test_project_skill_slash_invocation_runs_inline_session(tmp_path):
    skill_dir = tmp_path / ".iatcoder" / "skills" / "deploy"
...
---
Use target $ARGUMENTS from ${IATCODER_SKILL_DIR}.
""",
        encoding="utf-8",
    )
...
def test_memory_slash_commands_use_kairos_assets(tmp_path):
    ...
    (tmp_path / ".iatcoder" / "memory" / "MEMORY.md").write_text(
...
def test_dream_slash_command_consolidates_daily_log_into_memory_files(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            '<tool>{"name":"write_file","args":{"path":".iatcoder/memory/topics/user-preferences.md","content":"# User Preferences\\n\\n## Notes\\n- Prefers concise reports.\\n"}}</tool>',
    ...
    assert "User preferences" in (tmp_path / ".iatcoder" / "memory" / "MEMORY.md").read_text(encoding="utf-8")
    assert "Prefers concise reports" in (tmp_path / ".iatcoder" / "memory" / "topics" / "user-preferences.md").read_text(encoding="utf-8")
...
def test_skill_frontmatter_metadata_and_argument_substitution(tmp_path):
    skill_dir = tmp_path / ".iatcoder" / "skills" / "audit"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: audit
...
def test_fork_skill_runs_in_isolated_session_and_records_completion(tmp_path):
    skill_dir = tmp_path / ".iatcoder" / "skills" / "inspect"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: inspect
...
def test_skill_allowed_tools_restricts_inline_execution(tmp_path):
    skill_dir = tmp_path / ".iatcoder" / "skills" / "readonly"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: readonly
...
def test_disable_model_invocation_skill_returns_expanded_prompt(tmp_path):
    skill_dir = tmp_path / ".iatcoder" / "skills" / "template"
    skill_dir.mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text(
        """---
name: template
...
def test_cli_lists_skills_without_calling_model(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "iatcoder", "--cwd", str(tmp_path)],
        input="/skills\n/exit\n",
        text=True,
        capture_output=True,
        env=env,
