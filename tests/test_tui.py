...
def assistant_contents(app):
    from iatcoder.tui.widgets import AssistantMessage
...
def test_cli_defaults_interactive_tty_mode_to_tui(monkeypatch):
    from iatcoder.cli import build_arg_parser, interaction_mode

    monkeypatch.setattr(
        "iatcoder.cli.sys.stdin", type("Stdin", (), {"isatty": lambda self: True})()
    )
...
def test_cli_uses_plain_repl_for_piped_stdin(monkeypatch):
    ...
    monkeypatch.setattr(
        "iatcoder.cli.sys.stdin", type("Stdin", (), {"isatty": lambda self: False})()
    )
    args = build_arg_parser().parse_args(["--cwd", "/tmp/workspace"])
...
def test_status_bar_shows_runtime_identity(tmp_path):
    from iatcoder.tui.widgets import StatusBar
...
def test_cli_plan_mode_and_session_commands_expose_runtime_state(tmp_path):
    ...
    assert ".iatcoder/plans/refactor-auth-plan.md" in output
    assert agent.runtime_mode == "plan"

    handled, _, output = handle_repl_command(agent, "/mode")
    assert handled is True
    assert "runtime mode: plan" in output
    assert "plan path: .iatcoder/plans/refactor-auth-plan.md" in output
...
def test_slash_command_registry_suggests_and_parses_subagent():
    from iatcoder.commands.slash import (
        parse_subagent_args,
        resolve_command,
        suggest_commands,
    )
...
async def test_tui_slash_suggestions_complete_partial_command(tmp_path):
    ...
    from iatcoder.tui.widgets import InputBar, SlashSuggestions

    app = IatcoderTuiApp(build_agent(tmp_path, []))
...
async def test_tui_help_command_uses_existing_repl_commands(tmp_path):
    from iatcoder.tui.app import IatcoderTuiApp
    from iatcoder.tui.widgets import InputBar

    agent = build_agent(tmp_path, [])
    app = IatcoderTuiApp(agent)
...
async def test_tui_renders_tool_card_result(tmp_path):
    from iatcoder.tui.app import IatcoderTuiApp
    from iatcoder.tui.widgets import InputBar, ToolCard
...
async def test_tui_approval_prompt_controls_risky_tool(tmp_path):
    from iatcoder.tui.app import IatcoderTuiApp
    from iatcoder.tui.widgets import ConfirmPrompt, InputBar
...
async def test_tui_ask_user_prompt_returns_selected_choice(tmp_path):
    from iatcoder.tui.app import IatcoderTuiApp
    from iatcoder.tui.widgets import AskUserPrompt, InputBar
